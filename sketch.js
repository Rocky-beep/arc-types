//note: how to change CSS elements at different times 
//https://happycoding.io/tutorials/p5js/web-dev 
//https://www.youtube.com/watch?v=x_i9Rg3Ki54 
//https://www.dummies.com/article/business-careers-money/business/marketing/video-on-your-website-how-to-place-an-embed-code-175902
...
let primSubjects = ["Astronaut","Technology","Monsters","Dismembered Body Parts","Food","Location","Destruction"];

let colours = ["Multi","BW + 1","BW","2 colours"];

let space = ["Balanced","Packed","Open"];

let shiannMotifs = ["eyes","melting","slime","teeth","faceless features","machines","null"];
...
function preload() {
  ...
  imageData = loadTable('Nias_RTA82A_DataBase - DATABASEv3.csv', 'csv', 'header');
...
function displayText(arry, c){
  ...
  for(let i = 0; i < arry.length; i++){
    ...
    if((saturation(c) > 35 && brightness(c) < 90) || saturation(c) == 0 && brightness(c) == 0){
...
function ayePath(aa,ab,ba,bb){
  ...
  if(aa == 'b' || aa == 'l' || aa == 'u'){ //first name, first l
    needsMet = "excited"; 
  ...
  if(ab == 'b' || ab == 'l' || ab == 'u'){ //first name, last l
  ...
  if(ba == 'b' || ba == 'l' || ba == 'u'){ //last name, first l
  ...
  if(bb == 'b' || bb == 'l' || bb == 'u'){ //last name, last l
  ...
  let ocean = ["openess", "conscientiousness" , "extraversion", "agreeableness", "neuroticism"];
  
  let oceanConst = [4, 10, 7, 5, 6];
  
  for(let i = 0; i < enneaFin.length; i++){ //counting constanants 
...
function beePath(aa,ba){
  ...
  //Jungian archetype 
  let archs = ["Everyman","Ruler","Outlaw","Sage","Lover","Hero","Artist","Explorer","Jester","Caregiver","Magician","Innocent"];
  
  //corresponding color codes in hex 
  let colorCodes = ["#DF0B5D","#F45255","#EB6D00","#FFAA00","#F1D30E","#46D129","#3CBE72","#4EA798","#28BBFF","#178DE8","#385BAD","#A4469E"];
  let colorCode;
  ...
  if(numerology > 10 && numerology != 10 && numerology != 11 && numerology != 12){
    n = str(numerology);
  ...
  if(unchar(aa) <= 109){ 
    if(numerology != colorCodes.length && (numerology+1) != colorCodes.length){ //find the complimetary colour (next one) 
...
function ceePath(ab,bb){
  ...
  let trait;
  let mbti;
  let mbtiList = ["ISTJ", "ISFJ", "ISFP", "ISTP", "INFP", "INTP", "INFJ", "INTJ", "ESTJ", 
  "ESFJ", "ESFP", "ESTP", "ENFP", "ENTP", "ENFJ", "ENTJ"];
  ...
  for(let i = 0; i < hp.length; i++){
    ...
    if(!(hp[i] == "a" || hp[i] == ("e") || hp[i] == ("i") || hp[i] == ("o") || hp[i] == ("u") || hp[i] == ("y") || hp[i] == ("h") || hp[i] == ("w"))){
...
function phonicCipher(x,y){
  ...
  switch(x){
    ...
    case "n": p = "news reports";
    break;
    
    case "o": p = "outer spaces";
    break;
  ...
  switch(y){
    ...
    case "a": p += (" in The Abyss");
    abcPhonic = "Abyss";
    break;
    
    case "b": p += (" on The Blue Planet");
    abcPhonic = "BluePlanet";
    break; 
    
    case "c": p += (" in The Circus of Cacaesthesia");
    abcPhonic = "CircusCacaesthesia";
    break; 
    
    case "d": p += (" in The Deep, Dark Web");
    abcPhonic = "DeepDarkWeb";
    break;
    
    case "e": p += (" on Earth, the Second");
    abcPhonic = "EarthSecond";
    break; 
    
    case "f": p += (" on Fearless Avenue");
    abcPhonic = "FearlessAvenue";
    break;
    
    case "g": p += (" on The Green Planet");
    abcPhonic = "GreenPlanet";
    break;
    
    case "h": p += (" on Homer's Planet");
    abcPhonic = "HomersPlanet";
    break; 
    
    case "i": p += (" in The Icebox");
    abcPhonic = "Icebox";
    break;
    
    case "j": p += (" in The Junkyard");
    abcPhonic = "Junkyard";
    break;
    
    case "k": p += (" in Kazpr's House");
    abcPhonic = "KazprsHouse";
    break;
    
    case "l": p += (" in The LakeHouse");
    abcPhonic = "Lakehouse";
    break;
    
    case "m": p += (" in The Mill");
    abcPhonic = "Mill";
    break;
    
    case "n": p += (" in The Nowhere");
    abcPhonic = "Nowhere";
    break;
    
    case "o": p += (" on Planet Omen");
    abcPhonic = "OmenPlanet";
    break;
    
    case "p": p += (" in Purgatory");
    abcPhonic = "Purgatory";
    break;
    
    case "q": p += (" on Questionable Places");
    abcPhonic = "QuestionablePlaces";
    break;
    
    case "r": p += (" on The Red Planet");
    abcPhonic = "RedPlanet";
    break;
    
    case "s": p += (" in Sinkholes");
    abcPhonic = "Sinkholes";
    break;
    
    case "t": p += (" in Tomorrowville");
    abcPhonic = "Tomorrowville";
    break;
    
    case "u": p += (" in Universe Land");
    abcPhonic = "UniverseLand";
    break;
    
    case "v": p += (" in Vim Volcano Town");
    abcPhonic = "VimVolcanoTown";
    break;
    
    case "w": p += (" in The Who-Knows");
    abcPhonic = "WhoKnows";
    break;
    
    case "x": p += (" on A Xystus for Angels");
    abcPhonic = "XystusAngels";
    break;
    
    case "y": p += (" on The Yikker Planet");
    abcPhonic = "YikkerPlanet";
    break;
    
    case "z": p += (" in The Zizz Victims Unit");
...
function pick(index){
  ...
  if(index > primSubjects.length - 1){ //# - the largest the index
  ...
  if(spaceCorpse.length != 0){ //only change it if the list isn't empty 
    targId = spaceCorpse; 
    } else {
     return getResult(targId);
    }
  ...
  if(motifCorpse.length != 0){ //only change it if the list isn't empty 
    targId = motifCorpse; 
    } else {
     print(targId);
     return getResult(targId);
  ...
  if(shapeCorpse.length != 0){ //only change it if the list isn't empty 
    targId = shapeCorpse; 
    } else {
     return getResult(targId);
    }
...
function opening(){
  ...
  textSize(20);
  text("All the creatures and worlds that have ever existed are viewed under three major aspects. ", 10, 120);
  ...
  text("named after how the creatures responded to the initial discovery of TEEF the Eye-dol.", 20, 220);

  text("The Eye-dol inspired these three major feelings -- ", 20, height/2 + 230);
  text(" and everything that existed in the galaxy since then came from these creatures", 20, height/2 + 250);

  text("The galaxy says; these feelings didn't sound very fun and the creatures ultimately changed entirely; ", 20, height/2 + 300);
...
function rageDisplay(){
  ...
  rageText[0] = (fname + " " + lname + " gets the " + imageData.getString(imageIndex,"AnimName") + " as their Rage Alignment.");
  rageText[1] = ("In the phase of Rage this suggests that - much like the " + imageData.getString(imageIndex,"AnimName") + " - you have " + imageData.getString(imageIndex,"Verb 2") + ".");
  rageText[2] = ("Perhaps you identify with this.");
  rageText[3] = ("At least at the times you are angry and defiant of what is happening around you.");
  rageText[4] = ("Take this as a sign to recognize the types of things in your life that you want to change or bring you great frustration.");
...
function hysteriaDisplay(){
  ...
  hysteriaText[0] = (fname + " " + lname + " gets the " + imageData.getString(imageIndex1,"AnimName") + " as their Hysteria Alignment.");
  hysteriaText[1] = ("ask yourself, what in your life has been " + imageData.getString(imageIndex1,"Verb(ing)") + "?"); 
  hysteriaText[2] = ("And why do you fear it? Simultaneously, what part of it drives your morbid curiosity?");
  hysteriaText[3] = ("Is it the parts that are " + imageData.getString(imageIndex1,"Adjective") + " -esque?"); 
  hysteriaText[4] = ("Do you feel " + imageData.getString(imageIndex1,"State of being") + " whilst doing this thought exercise? So many questions, " + imageData.getString(imageIndex1,"Interjection"));
...
function despairDisplay(){
  ...
  despairText[0] = (fname + " " + lname + " gets the " + imageData.getString(imageIndex2,"AnimName") + " as their Despair Alignment.");
  despairText[1] = (imageData.getString(imageIndex2,"Phrase"));
  despairText[2] = ("This is it, you think, this is the thing that makes you feel " + imageData.getString(imageIndex2,"State of being 2") + " but not in the traditional sense, ");
  despairText[3] = ("But more in the way a " + imageData.getString(imageIndex2,"Noun") + " feels that.");
  despairText[4] = ("You are changing. You are " + imageData.getString(imageIndex2,"Verb 2") + " and it is " + imageData.getString(imageIndex2,"Adjective 2") + ".");
  despairText[5] = ("And no matter how " + imageData.getString(imageIndex2,"Emotion") + (" you feel, that feeling will change too. And somehow it will all be okay.")); 
...
function keyPressed(){ //resolve "enter" issue
